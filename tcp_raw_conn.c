...
void create_syn_packet(struct sockaddr_in* src, struct sockaddr_in* dst, char** out_packet, int* out_packet_len)
{
    ...
    tcph->check = checksum((unsigned short*)pseudogram, psize / 2); // TODO: call checksum() with pseudogram
        iph->check = checksum((unsigned short*)datagram, iph->tot_len / 2);
...
int main(int argc, char *argv[])
{
        ...
        {
                printf("Successfully received %d bytes SsockYN-ACK!\n", received);
        }
